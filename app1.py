...
st.write("Carga una imagen y el modelo entrenado en Teachable Machine realizará la predicción de forma local.")

# Deshabilitar advertencias innecesarias de desaprobación de hardware
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

# Función optimizada con caché utilizando el motor clásico v2 compatible con la nube
@st.cache_resource
def load_teachable_model():
    try:
        # Forzar el uso del motor de compatibilidad clásica en servidores modernos
        import tf_keras as keras
        return keras.models.load_model("keras_model.h5", compile=False)
    except ImportError:
        # Fallback estándar local si no está tf_keras
...
if model is not None and len(class_names) > 0:
    ...
    # Componente nativo para subir archivos de imagen
    uploaded_file = st.file_uploader("Selecciona una imagen (JPG, JPEG o PNG)...", type=["jpg", "jpeg", "png"])

    if uploaded_file is not None:
        # Abrir y previsualizar la imagen del usuario
        ...
        # --- MOSTRAR RESULTADOS METICULOSAMENTE ---
        st.markdown("---")
        ...
        # Mostrar el desglose completo de probabilidades mediante un gráfico interactivo
        st.write("### Desglose de Probabilidades:")
        chart_data = {class_names[i]: float(prediction[0][i]) for i in range(len(class_names))}
        st.bar_chart(chart_data)
else:
    st.warning("⚠️ Asegúrate de que tanto 'keras_model.h5' como 'labels.txt' se encuentren en la ruta actual de la consola.")